...
bool TimerQueue::addTimer(Timer* time)
{
...
int64_t TimerQueue::waitDurationUsec(int64_t duration) const
{
    if (heap_.empty())
    {
        return duration;
...
std::size_t TimerQueue::cancelTimer(Timer* timer)
{
    removeTimer(timer);
    return 0;
}
...
std::list<Timer*> TimerQueue::getReadyTimers()
{
...
void TimerQueue::upHeap(std::size_t index)
{
    while (index > 0)
    {
...
void TimerQueue::downHeap(std::size_t index)
{
...
void TimerQueue::swapHeap(std::size_t index1, std::size_t index2)
{
